<template>
  <div class="basic-container unicom-table">
    <el-card class="inquire-header" v-if="vaildData(option.searchFlag, true)">
      ...
      <div class="unicom-bottom" v-if="!formWidth">
        <el-button
          type="primary"
          @click="searchChnage"
          icon="el-icon-search"
    ...
    <el-card
      :style="
        'margin-top:' + (option.headerBottom ? +option.headerBottom : 0) + 'px'
      "
    >
      <!-- 表格功能列 -->
        ...
        <div class="unicom-munu">
          ...
          <div class="unicom-menu_right">
            ...
            <el-button
              ...
              size="small"
              @click="refreshChange"
              v-if="vaildData(option.refreshBtn, false)"
            ></el-button>
            <el-button
              icon="el-icon-menu"
              circle
              ...
              v-if="vaildData(option.showClomnuBtn, false)"
            ></el-button>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column
          v-if="showClomnuIndex.indexOf(index) != -1"
        ...
        >
          <template slot-scope="scope">
            ...
            <template v-else>
              <span
                v-html="detail(scope.row, column)"
                v-if="column.type"
              ></span>
              <span v-else>
                {{
                  ...
                  !validatenull(column.valueDefault)
                    ? column.valueDefaul
                    : scope.row[column.prop]
                }}
              </span>
      ...
      >
          ...
          <el-row :gutter="20" :span="24">
            <template v-for="(column, index) in option.column">
              <el-col
                ...
                v-if="
                  boxType == 0
                    ? vaildData(column.addVisdiplay, true)
                    : vaildData(column.editVisdiplay, true)
                "
              >
                <el-form-item
                  ...
                  :prop="column.prop"
                  :label-width="
                    setPx(column.labelWidth, option.labelWidth || 120)
                  "
                >
                  <slot
                  ...
                  <component
                    ...
                    :disabled="
                      boxType == 0
                        ? vaildData(column.addDisabled, false)
                        : vaildData(column.editDisabled, false)
                    "
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="handleUpdate" v-if="boxType == 1"
            >修 改</el-button
          >
    ...
    <el-card v-if="vaildData(option.selected, true)" class="selected">
      <span class="message">
        <i class="el-icon-warning"></i>已选择{{ tableSelect.length }}项
      </span>
      <span class="clear" @click="handleClearSelected">清空</span>
    </el-card>
    <el-dialog
      :title="option.textMap"
  ...
  methods: {
    ...
    formInit() {
      ...
      this.searchForm = Object.assign(
        ...
        this.defaultValue && this.defaultSearch(this.defaultValue)
      );
    },
    defaultSearch(defalut) {
      let type = Object.prototype.toString.call(defalut);
      if (type === "[object Array]") {
        ...
        return { [defalut.key]: defalut.value };
      } else {
        return defalut;
      }
    },
    ...
    searchReset() {
      ...
      this.$parent.handleGetCenterChildDept &&
        this.$parent.handleGetCenterChildDept();
    ...
    handleText(val, type) {
      this.$nextTick(() => {
        switch (type) {
          case "userPhone":
            this.$nextTick(() => {
              this.$refs.userPhone[0].text = this.$refs.userPhone[0].text.replace(
                /\D/g,
                ""
              );
              if (val.length > 11) {
          ...
          case "contactPhone":
            this.$nextTick(() => {
              this.$refs.contactPhone[0].text = this.$refs.contactPhone[0].text.replace(
                /\D/g,
                ""
              );
              if (val.length > 11) {
          ...
          case "phone":
            this.$nextTick(() => {
              this.$refs.phone[0].text = this.$refs.phone[0].text.replace(
                /\D/g,
                ""
              );
              if (val.length > 11) {
          ...
          case "caseId":
            this.$nextTick(() => {
              this.$refs.caseId[0].text = this.$refs.caseId[0].text.replace(
                /\D/g,
                ""
              );
            });
          ...
          case "overdueDayMin":
            this.$nextTick(() => {
              this.$refs.overdueDayMin[0].text = this.$refs.overdueDayMin[0].text.replace(
                /\D/g,
                ""
              );
            });
          ...
          case "overdueDayMax":
            if (!res) {
              this.$nextTick(() => {
                this.$refs.overdueDayMax[0].text = this.$refs.overdueDayMax[0].text.replace(
                  /\D/g,
                  ""
                );
              });
