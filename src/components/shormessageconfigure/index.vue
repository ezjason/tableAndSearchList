<template>
  <div class="super">
    ...
    <van-calendar v-model="show" type="range" :default-date="null"/>
    <unicom-tbale
      :data="tableData"
      :page="page"
      ref="super"
    ...
    >
      ...
      <template slot="menu" slot-scope="scope">
        ...
        >
          <el-button
            :type="scope.row.enabled == 1 ? 'danger' : 'warning'"
    ...
    <el-dialog :title="addType" :width="'398px'" :visible.sync="allotVisible">
      <el-form
        :model="allot"
        ref="allot"
        :rules="allotRules"
      ...
      >
        ...
        <el-form-item prop="minovertime" label="逾期：">
          <el-input
            v-model="allot.minovertime"
            placeholder="最小逾期天数"
            maxlength="10"
            @keyup.native="numberVal(allot.minovertime, 1)"
          ></el-input>
        </el-form-item>
        <el-form-item prop="maxovertime" label="">
          <el-input
            v-model="allot.maxovertime"
            placeholder="最大逾期天数"
            maxlength="10"
            @keyup.native="numberVal(allot.minovertime, 2)"
          ></el-input>
        </el-form-item>
        <el-form-item prop="textarea" label="模板：">
          <el-input
            type="textarea"
            v-model="allot.textarea"
            placeholder="模板"
            :disabled="edit"
            :autosize="{ minRows: 4 }"
          ></el-input>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleHideAllot">保存</el-button>
        <el-button @click="allotVisible = false">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleList(form) {
      ...
      ApiAllBorrowTypeList(param).then((res) => {
        res = res.data;
        if (res.code === 200) {
    ...
    handleGetCenterTree() {
      getCenterTree({}).then((res) => {
        this.$set(
          this.tableOption.dicData,
          "CENTERNAME",
          this.$setoption(res.data, "name", "id")
        );
      ...
      getFindAllBorrowTypeList({}).then((res) => {
        if (res.data.code === 200) {
          this.$set(
            this.tableOption.dicData,
            "CASETYPE",
            this.$setoption(res.data.data, "borrowTypeName", "borrowType")
          );
    ...
    handleHideAllot() {
      ...
      this.$refs.allot.validate((valid) => {
        if (valid) {
          if (
            parseInt(this.allot.minovertime) > parseInt(this.allot.maxovertime)
          ) {
            this.$message.error(
              "最小逾期天数不能大于最大逾期天数，请重新填写！"
            );
            return;
          }
    ...
    numberVal(val, type) {
      ...
      if (!rel.test(val)) {
        type == 1
          ? (this.allot.minovertime = "")
          : (this.allot.maxovertime = "");
        this.$message("请输入数字！");
      }
    },
  },
